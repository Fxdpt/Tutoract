...
class UserController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $em, UserPasswordEncoderInterface $passwordEncoder, PromoRepository $promoRepository, UserRepository $userRepository, TechRepository $techRepository)
    {
        ...
        if ($emailUsers != null) {
            $success = false;
            $response = JsonHandler::responseHandler($success, 'Cet adresse email possède déjà un compte');
        ...
        if ($isFoundInOclockApi && $data['password'] === $data['confirm_password']) {
            ...
            $success = true;
            $response = JsonHandler::responseHandler($success, 'Inscription validée !');
            return $this->json($response);
        }

        $success = false;
        $response = JsonHandler::responseHandler($success, 'Cette adresse email n\'est pas autorisé à se créer un compte');
        return $this->json($response);
    }
    ...
    public function edit(User $user, EntityManagerInterface $em, SerializerInterface $serializer, Request $request)
    {
        ...
        $success = true;
        $response = JsonHandler::responseHandler($success, 'Profil mis à jour');
        return $this->json($response);
    }
    ...
    public function uploadAvatar(ImageUploader $imageUploader, Request $request)
    {
        ...
        if(!$filesArray['avatar']){
            $success = false;
            $response = JsonHandler::responseHandler($success, 'Vous n\'avez pas choisi d\'avatar');
            return $this->json($response);
        }else{
            $user = $this->getUser();
            ...
            if ($form->isSubmitted() && $form->isValid()) {
                ...
                $success = true;
                $response = JsonHandler::responseHandler($success, 'Avatar mis à jour');
                return $this->json($response);
            }    
        }
        
        $success = false;
        $response = JsonHandler::responseHandler($success, 'Votre avatar n\'a pas pu être mis à jour');
        return $this->json($response);
    }
    ...
    public function delete(User $user, EntityManagerInterface $em)
    {
        ...
        $success = true;
        $response = JsonHandler::responseHandler($success, 'Profil supprimé');
        return $this->json($response);
    }
