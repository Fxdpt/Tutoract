...
class EventController extends AbstractController
{
    ...
    public function addUser(Event $event, SerializerInterface $serializer, UserRepository $userRepository, Request $request)
    {
        ...
        foreach($event->getUsers() as $student){
            if($user === $student){
                $success = false;
                $response = JsonHandler::responseHandler($success,'Vous êtes déjà inscrit');
        ...
        $em->flush();
        $success = true;
        $response = JsonHandler::responseHandler($success,'Vous êtes bien inscrit');
        return $this->json($response);
    ...
    public function new(Request $request, UserRepository $userRepository, TechRepository $techRepository)
    {
        ...
        $success = true;
        $response = JsonHandler::responseHandler($success,'Votre évènement est créé');
        return $this->json($response);
    }
    ...
    public function edit(Event $event, Request $request, SerializerInterface $serializer, EntityManagerInterface $em, UserRepository $userRepository)
    {
        ...
        if($event->getOwner() !== $user){
            $success = false;
            $response = JsonHandler::responseHandler($success,'Vous n\'êtes pas le propriétaire de cet évènement');
            return $this->json($response);
        }
        ...
        $success = true;
        $response = JsonHandler::responseHandler($success,'Evènement édité');
        return $this->json($response);
    }
    ...
    public function delete(Event $event, Request $request)
    {
        ...
        $success = true;
        $response = JsonHandler::responseHandler($success,'Evènement supprimé'); 
        return $this->json($response);
    }
    ...
    public function addMessage(Event $event, SerializerInterface $serializer, Request $request)
    {
        ...
        $success = true;
        $response = JsonHandler::responseHandler($success,'Message envoyé');
        return $this->json($response);
    }
}
